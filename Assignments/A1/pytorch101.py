...
def multiples_of_ten(start, stop):
  ...
  for n in range(start, stop):
    if n % 10 == 0:
      m.append(n)
  x = torch.tensor(m, dtype=torch.float64)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def slice_assignment_practice(x):
  ...
  # Replace "pass" statement with your code
  for i in range(2):
    x[i, :6] = torch.tensor([0, 1, 2, 2, 2, 2])
  for i in range(2, 4):
    x[i, :6] = torch.tensor([3, 4, 3, 4, 5, 5])
...
def normalize_columns(x):
  ...
  n = x.shape[0]
  mean = x.sum(dim=0) / n
  std = (((x - mean)**2).sum(dim=0) / (n - 1)).sqrt()
  y = (x - mean)/std
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
