...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
    idists = ((x_test - x_train[i])**2).sum(dim=(1,2,3))
    dists[i] = idists
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  # I found hint at:https://stackoverflow.com/questions/27948363/numpy-broadcast-to-perform-euclidean-distance-vectorized
  A = x_train.reshape(num_train,-1)
  B = x_test.reshape(num_test,-1)
  AB2 = A.mm(B.t()) * 2
  dists = (A * A).sum(dim=1).view(-1, 1) + (B * B).sum(dim=1) - AB2
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  # Get the indices of min-k-distance training data. 
  # The min_k_ind is a kxnum_test matrix
  min_k_ind = dists.topk(k, dim=0, largest=False).indices
  # For each column of min_k_ind, we can get the corresponding labels in y_train
  # Then we get a kxnum_test matrix of labels
  # For each column of labels we can count the labels, using torch.bincount
  # e.g. [1, 1, 0] ->[1, 2]
  labels = y_train[min_k_ind]
  for i in range(num_test):
    freq, label = labels[:, i].bincount().max(dim=0)
    y_pred[i] = label
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_folds = x_train.chunk(chunks=num_folds, dim=0)
  y_train_folds = y_train.chunk(chunks=num_folds, dim=0)
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  ...
  for k in k_choices:
    #Each k gets a list of accs for different testing fold
    folds_accuracies = []
    #For different chosen fold
    for i in range(num_folds):
      ...
      #Calculate the accuracy corrsponding to different k and fold
      accuracy = classifier.check_accuracy(x_test, y_test, k)
      folds_accuracies.append(accuracy)
    k_to_accuracies[k] = folds_accuracies
