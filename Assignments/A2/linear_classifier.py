...
def svm_loss_naive(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  # Remember to divide by N as we are doing gradients on loss!!!
  dW /= num_train
  dW += 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margin = scores - correct_class_scores.view(-1, 1) + 1.0
  #Remember to exclude the right class scor es in loss computing
  margin[range(num_train), y] = 0.0
  #Remember to add the regularization term
  loss += margin[margin>0.0].sum() / num_train + reg * torch.sum(W * W)
  print(loss)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  index = torch.randint(low=0, high=num_train-1, size=(batch_size,))
  X_batch = X[index]
  y_batch = y[index]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  y_pred = X.matmul(W).argmax(dim=1)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.00542 + 0.0000001*x for x in range(51,56)]
  regularization_strengths = [0.00001*x for x in range(1,6)]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def compute_acc(haty, y, n):
  diff_y = y - haty
  diff_y[diff_y!=0] = 1
  correct_train_y = n - diff_y.sum()
  acc = correct_train_y / n
  return acc
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  hat_train_y = cls.predict(X_train)
  train_acc = compute_acc(hat_train_y, y_train, num_train)

  hat_val_y = cls.predict(X_val)
  val_acc = compute_acc(hat_val_y, y_val, num_val)
